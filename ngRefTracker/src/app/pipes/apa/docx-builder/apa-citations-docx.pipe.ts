...
import { ApaCitationDocxPipe } from './apa-citation-docx.pipe';

@Pipe({
  name: 'apaCitationsDocx'
})
export class ApaCitationsDocxPipe implements PipeTransform {

  transform(articlesToCite: JournalArticle[]): Paragraph[] {
    let toReturn: Paragraph[] = [];
    for (let i=0; i < articlesToCite.length; i++) {
      toReturn.push(new ApaCitationDocxPipe().transform(articlesToCite[i]));
